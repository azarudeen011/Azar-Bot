...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!type || !["standings", "scorers", "matches", "upcoming"].includes(type)) {
        let helpText = `🏆 *UEFA CHAMPIONS LEAGUE - FOOTBALL*\n\nUsage: ${settings.prefix}ucl <option>\n\n*Available Options:*\n`;
    ...
    try {
        ...
        const url = `https://api.princetechn.com/api/football/ucl/${endpoint}?apikey=prince`;
        const res = await axios.get(url, { timeout: 30000 });

        if (!res.data || !res.data.success || !res.data.result) {
             return sock.sendMessage(from, { text: `❌ Failed to fetch UCL ${type}.` }, { quoted: msg });
        }

        const data = res.data.result;
        let text = `🏆 *UCL: ${type.toUpperCase()}* 🏆\n\n`;
        ...
        console.log("UCL Error:", e.message);
        await sock.sendMessage(from, { text: `❌ API Error fetching UCL ${type}.` }, { quoted: msg });
    }
};
