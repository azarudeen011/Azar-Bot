...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!type || !["standings", "scorers", "matches", "upcoming"].includes(type)) {
        let helpText = `🇪🇸 *LA LIGA - FOOTBALL*\n\nUsage: ${settings.prefix}laliga <option>\n\n*Available Options:*\n`;
    ...
    try {
        ...
        const url = `https://api.princetechn.com/api/football/laliga/${endpoint}?apikey=prince`;
        const res = await axios.get(url, { timeout: 30000 });

        if (!res.data || !res.data.success || !res.data.result) {
             return sock.sendMessage(from, { text: `❌ Failed to fetch La Liga ${type}.` }, { quoted: msg });
        }

        const data = res.data.result;
        let text = `🇪🇸 *LA LIGA: ${type.toUpperCase()}* 🇪🇸\n\n`;
        ...
        console.log("La Liga Error:", e.message);
        await sock.sendMessage(from, { text: `❌ API Error fetching La Liga ${type}.` }, { quoted: msg });
    }
};
