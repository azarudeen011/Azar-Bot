...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!type || !["standings", "scorers", "matches", "upcoming"].includes(type)) {
        let helpText = `🇩🇪 *BUNDESLIGA - FOOTBALL*\n\nUsage: ${settings.prefix}bundesliga <option>\n\n*Available Options:*\n`;
    ...
    try {
        ...
        const url = `https://api.princetechn.com/api/football/bundesliga/${endpoint}?apikey=prince`;
        const res = await axios.get(url, { timeout: 30000 });

        if (!res.data || !res.data.success || !res.data.result) {
             return sock.sendMessage(from, { text: `❌ Failed to fetch Bundesliga ${type}.` }, { quoted: msg });
        }

        const data = res.data.result;
        let text = `🇩🇪 *BUNDESLIGA: ${type.toUpperCase()}* 🇩🇪\n\n`;
        ...
        console.log("Bundesliga Error:", e.message);
        await sock.sendMessage(from, { text: `❌ API Error fetching Bundesliga ${type}.` }, { quoted: msg });
    }
};
