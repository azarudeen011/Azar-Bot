...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!text) {
        return sock.sendMessage(from, { text: "🎨 *LOGO MAKER*\n\nUsage: " + settings.prefix + "gradienttext <text>\nExample: " + settings.prefix + "gradienttext AzarTech" }, { quoted: msg });
    }

    try {
        await sock.sendMessage(from, { react: { text: "⏳", key: msg.key } }).catch(() => { });

        const url = `https://api.princetechn.com/api/ephoto360/gradienttext?apikey=prince&text=${encodeURIComponent(text)}`;
        ...
        const caption = `🎨 *LOGO GENERATED: GRADIENTTEXT* 🎨\n\n> ${small_lib.author}`;

        await sock.sendMessage(from, { image: { url: imageUrl }, caption }, { quoted: msg });

    } catch (e) {
        console.log("gradienttext Error:", e.message);
