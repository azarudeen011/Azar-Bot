...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!text) {
        return sock.sendMessage(from, { text: "🎨 *LOGO MAKER*\n\nUsage: " + settings.prefix + "glossysilver <text>\nExample: " + settings.prefix + "glossysilver AzarTech" }, { quoted: msg });
    }
    ...
    try {
        ...
        const caption = `🎨 *LOGO GENERATED: GLOSSYSILVER* 🎨\n\n> ${small_lib.author}`;

        await sock.sendMessage(from, { image: { url: imageUrl }, caption }, { quoted: msg });

    } catch (e) {
        console.log("glossysilver Error:", e.message);
