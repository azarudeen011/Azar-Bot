...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!text) {
        return sock.sendMessage(from, { text: "🎨 *LOGO MAKER*\n\nUsage: " + settings.prefix + "writetext <text>\nExample: " + settings.prefix + "writetext AzarTech" }, { quoted: msg });
    }

    try {
        await sock.sendMessage(from, { react: { text: "⏳", key: msg.key } }).catch(() => { });

        const url = `https://api.princetechn.com/api/ephoto360/writetext?apikey=prince&text=${encodeURIComponent(text)}`;
        ...
        const caption = `🎨 *LOGO GENERATED: WRITETEXT* 🎨\n\n> ${small_lib.author}`;

        await sock.sendMessage(from, { image: { url: imageUrl }, caption }, { quoted: msg });

    } catch (e) {
        console.log("writetext Error:", e.message);
