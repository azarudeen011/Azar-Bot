...
module.exports = async (sock, msg, from, body, args) => {
    ...
    if (!text) {
        return sock.sendMessage(from, { text: "🎨 *LOGO MAKER*\n\nUsage: " + settings.prefix + "1917 <text>\nExample: " + settings.prefix + "1917 AzarTech" }, { quoted: msg });
    }

    try {
        await sock.sendMessage(from, { react: { text: "⏳", key: msg.key } }).catch(() => { });

        const url = `https://api.princetechn.com/api/ephoto360/1917?apikey=prince&text=${encodeURIComponent(text)}`;
        ...
        const caption = `🎨 *LOGO GENERATED: 1917* 🎨\n\n> ${small_lib.author}`;

        await sock.sendMessage(from, { image: { url: imageUrl }, caption }, { quoted: msg });

    } catch (e) {
        console.log("1917 Error:", e.message);
